...
def format_full_analysis(results: dict) -> str:
    ...
    if htf and "error" not in htf:
        ...
        sym_hint = next((d["symbol"] for tf, d in results.items()
                         if not tf.startswith("__") and isinstance(d, dict) and "error" not in d and "symbol" in d), "")
        if sym_hint:
...
def format_ob_fvg_only(results: dict) -> str:
    ...
    lines = []
    for tf, data in results.items():
        if tf.startswith("__") or "error" in data:
            continue
...
def format_report(results: dict) -> str:
    ...
    htf = results.get("__htf__", {})
    sym_hint = next((d["symbol"] for tf, d in results.items()
                     if not tf.startswith("__") and isinstance(d, dict) and "error" not in d and "symbol" in d), "COIN")
    ...
    lines.append("_Report generated by Crypto Market Analyst Bot v2.0_")
    return "\n".join(lines)


# ═══════════════════════════════════════════════════════════════
# NEW v2.0 FORMATTER FUNCTIONS
# ═══════════════════════════════════════════════════════════════

def format_harmonic_only(results: dict) -> str:
    """Harmonic pattern analysis — /harmonic command."""
    lines = []
    sym = None
    ...
    for tf, data in results.items():
        ...
        if sym is None:
            sym = data["symbol"]
            lines.append(f"🔷 *{sym}/USDT — Harmonic Patterns*\n")

        harmonic = data.get("harmonic", {})
        price = fmt_price(data["current_price"])

        lines.append(f"`{tf}` · {price}")

        if not harmonic or harmonic.get("count", 0) == 0:
            lines.append("  No harmonic patterns detected.")
            lines.append("")
            continue

        lines.append(f"  Found: *{harmonic['count']}* patterns  ·  Bias: *{harmonic['bias'].upper()}*")
        lines.append("")

        for p in harmonic.get("patterns", []):
            emoji = p.get("emoji", "🔷")
            name = p["name"]
            direction = "🟢 Bullish" if p["direction"] == "bullish" else "🔴 Bearish"
            reliability = p.get("reliability", "moderate").upper()

            lines.append(f"  {emoji} *{name}* — {direction}")
            lines.append(f"  Reliability: _{reliability}_")
            lines.append(f"  Points:")
            pts = p["points"]
            lines.append(f"    X={fmt_price(pts['X'])}  A={fmt_price(pts['A'])}")
            lines.append(f"    B={fmt_price(pts['B'])}  C={fmt_price(pts['C'])}")
            lines.append(f"    D={fmt_price(pts['D'])}")
    ...
    if not lines:
        lines.append("⚠️ No data available for harmonic analysis.")

    return "\n".join(lines)


def format_whale_only(results: dict) -> str:
    """Order flow / whale activity — /whale command."""
    lines = []
    sym = None
    ...
    for tf, data in results.items():
        ...
        if sym is None:
            sym = data["symbol"]
            lines.append(f"🐋 *{sym}/USDT — Whale Activity Report*\n")
        ...
        if not of:
            lines.append("  No order flow data available.")
            lines.append("")
            continue

        whale_score = of.get("whale_score", 0)
        whale_label = of.get("whale_label", "N/A")
        bias = of.get("bias", "neutral").upper()

        # Whale score meter
        filled = "█" * (whale_score // 10)
        empty = "░" * (10 - whale_score // 10)
        lines.append(f"  Score: `{filled}{empty}` *{whale_score}/100*")
        lines.append(f"  {whale_label}")
        lines.append(f"  Bias: *{bias}*")
        lines.append("")

        # Accumulation/Distribution
        ad = of.get("accum_dist", {})
        if ad.get("label", "N/A") != "N/A":
            lines.append(f"  {ad['label']}")
            lines.append("")

        # Events
        events = of.get("events", [])
        if events:
        ...
        # Stats
        lines.append(f"  Climax: {of.get('climax_count', 0)} · Absorption: {of.get('absorption_count', 0)}")
        lines.append(f"  Exhaustion: {of.get('exhaustion_count', 0)} · Large Players: {of.get('large_player_count', 0)}")
        lines.append("")
        lines.append("─" * 30)
        lines.append("")

    if not lines:
        lines.append("⚠️ No order flow data available.")

    return "\n".join(lines)


def format_ml_only(results: dict) -> str:
    """ML prediction — /ml command."""
    lines = []
    sym = None
    ...
    for tf, data in results.items():
        ...
        if sym is None:
            sym = data["symbol"]
            lines.append(f"🤖 *{sym}/USDT — ML Price Prediction*\n")
        ...
        if not ml or ml.get("prediction") == "N/A":
            error = ml.get("error", "ML not available") if ml else "ML not available"
        ...
        lookahead = ml.get("lookahead", 3)

        lines.append(f"  Direction: *{prediction}*")
        lines.append(f"  Confidence: *{confidence}%*")
        lines.append(f"  Model Accuracy: *{accuracy}%* (5-fold CV)")
        lines.append(f"  Lookahead: {lookahead} candles")
        lines.append(f"  Training Samples: {ml.get('training_samples', 'N/A')}")
        lines.append(f"  Features Used: {ml.get('total_features', 'N/A')}")
        lines.append("")
        ...
        if features:
            ...
            for f in features[:5]:
                bar = "█" * max(1, int(f["importance"] / 5))
                lines.append(f"  `{bar}` {f['name']} ({f['importance']}%)")
            lines.append("")

        # Probability breakdown
        up_prob = ml.get("up_probability", 0)
        down_prob = ml.get("down_probability", 0)
        lines.append(f"  📈 UP probability: {up_prob}%")
        lines.append(f"  📉 DOWN probability: {down_prob}%")
        lines.append("")
        lines.append("─" * 30)
        lines.append("")

    if not lines:
        lines.append("⚠️ No ML prediction data available.")

    lines.append("_⚠️ ML predictions are statistical — not financial advice._")
    return "\n".join(lines)


def format_news_only(news_data: dict) -> str:
    """News sentiment — /news command."""
    lines = []

    if not news_data or news_data.get("error"):
        error = news_data.get("error", "News unavailable") if news_data else "News unavailable"
        return f"📰 *News Sentiment*\n\n⚠️ {error}"

    symbol = news_data.get("symbol", "CRYPTO")
    lines.append(f"📰 *{symbol} — News Sentiment Analysis*\n")

    label = news_data.get("label", "N/A")
    lines.append(f"  {label}")
    lines.append(f"  Confidence: *{news_data.get('confidence', 0)}%*")
    lines.append(f"  Bull signals: {news_data.get('bullish_total', 0)} · Bear signals: {news_data.get('bearish_total', 0)}")
    lines.append(f"  Total articles: {news_data.get('total_articles', 0)} · Relevant: {news_data.get('relevant_articles', 0)}")
    lines.append("")
    ...
    if articles:
        ...
        for a in articles[:8]:
            sentiment_icon = "🟢" if a["sentiment"] == "bullish" else "🔴" if a["sentiment"] == "bearish" else "⚪"
            lines.append(f"  {sentiment_icon} {a['emoji']} _{a['title'][:80]}_")
            lines.append(f"    Source: {a['source']} · Score: +{a['bull_score']}/-{a['bear_score']}")
        lines.append("")

    if not articles:
        lines.append("  No relevant news articles found.")

    lines.append("_News data from CoinDesk, CoinTelegraph, Bitcoin Magazine_")
    return "\n".join(lines)


def format_session_info(session_data: dict) -> str:
    """Current trading session — /session command."""
    if not session_data or "error" in session_data:
        return "⏰ *Trading Session*\n\n⚠️ Session detection unavailable."

    lines = [
        f"⏰ *Trading Session Info*\n",
        f"  {session_data.get('label', 'Unknown')}",
        f"  UTC Time: `{session_data.get('utc_time', 'N/A')}`",
        "",
        f"  📊 Volatility: *{session_data.get('volatility', 'N/A')}*",
        f"  💧 Liquidity: *{session_data.get('liquidity', 'N/A')}*",
        f"  📈 Best for: _{session_data.get('best_for', 'N/A')}_",
        "",
        f"  _{session_data.get('description', '')}_",
        "",
        f"  Confidence Adjustment: `{session_data.get('confidence_adjustment', 0):+d}`",
    ]

    active = session_data.get("active_sessions", [])
    if len(active) > 1:
        labels = [s.replace("_", " ").title() for s in active]
        lines.append(f"  Active: {', '.join(labels)}")

    return "\n".join(lines)


def format_heatmap(results: dict) -> str:
    """Multi-TF trend heatmap — /heatmap command."""
    lines = []
    sym = None
    ...
    for tf, data in results.items():
        ...
        tf_trends[tf] = {
            ...
            "ml": ml.get("prediction", "N/A") if ml else "N/A",
        }

    if not tf_trends:
        return "⚠️ No data for heatmap."

    lines.append(f"🗺 *{sym}/USDT — Multi-TF Trend Heatmap*\n")

    # Grid
    for tf, info in tf_trends.items():
        score_bar = "█" * abs(info["score"]) + "░" * (10 - abs(info["score"]))
        lines.append(f"  {info['emoji']} `{tf:4s}` `{score_bar}` *{info['score']:+d}/10*  {info['bias']}")
        lines.append(f"       {info['trend'].split('(')[0].strip()}")
        if info['ml'] != "N/A":
            lines.append(f"       ML: {info['ml']}")
        lines.append("")

    # Overall alignment check
    scores = [v["score"] for v in tf_trends.values()]
    if all(s > 0 for s in scores):
        lines.append("✅ *All timeframes aligned BULLISH*")
    elif all(s < 0 for s in scores):
        lines.append("✅ *All timeframes aligned BEARISH*")
    elif all(s == 0 for s in scores):
        lines.append("⚪ *All timeframes NEUTRAL*")
    else:
        lines.append("⚠️ *Mixed signals across timeframes*")

    return "\n".join(lines)


def format_indicators_only(results: dict) -> str:
    """Full indicator dashboard — /indicators command."""
    lines = []
    sym = None
    ...
    for tf, data in results.items():
        ...
        if sym is None:
            sym = data["symbol"]
            lines.append(f"📈 *{sym}/USDT — Indicator Dashboard*\n")
        ...
        # Core indicators
        lines.append("*Core:*")
        lines.append(f"  RSI: {rsi.get('value', 'N/A')} · {rsi.get('zone', '')} · {rsi.get('direction', '')}")
        lines.append(f"  MACD: {macd.get('momentum', '')} · Hist {macd.get('histogram_trend', '')}")
        if macd.get("crossover", "None") != "None":
            lines.append(f"    ↳ {macd['crossover']}")
        lines.append(f"  EMA: {ema.get('trend', '').split('(')[0].strip()}")
        lines.append(f"  Stoch: %K={stoch.get('k', 0)} %D={stoch.get('d', 0)} · {stoch.get('zone', '')}")
        lines.append(f"  Volume: {vol.get('trend', '').split('(')[0].strip()}")
        lines.append("")

        # Extended indicators
        if ext:
            lines.append("*Extended:*")
            ichimoku = ext.get("ichimoku", {})
            if ichimoku.get("signal", "N/A") != "N/A":
                lines.append(f"  Ichimoku: {ichimoku['signal']}")
            st = ext.get("supertrend", {})
            if st.get("direction", "N/A") != "N/A":
                lines.append(f"  Supertrend: {st['direction']} @ {fmt_price(st.get('value', 0))}")
            cci = ext.get("cci", {})
            if cci.get("zone", "N/A") != "N/A":
                lines.append(f"  CCI: {cci.get('value', 0)} · {cci['zone']}")
            wr = ext.get("williams_r", {})
            if wr.get("zone", "N/A") != "N/A":
                lines.append(f"  Williams %R: {wr.get('value', 0)} · {wr['zone']}")
            mfi = ext.get("mfi", {})
            if mfi.get("zone", "N/A") != "N/A":
                lines.append(f"  MFI: {mfi.get('value', 0)} · {mfi['zone']}")
            cmf = ext.get("cmf", {})
            if cmf.get("label", "N/A") != "N/A":
                lines.append(f"  CMF: {cmf.get('value', 0)} · {cmf['label']}")
            vwap = ext.get("vwap", {})
            if vwap.get("position", "N/A") != "N/A":
                lines.append(f"  VWAP: {fmt_price(vwap.get('value', 0))} · {vwap['position']}")
            obv = ext.get("obv", {})
            if obv.get("trend", "N/A") != "N/A":
                lines.append(f"  OBV: {obv['trend']}")
            lines.append("")
    ...
    if not lines:
        lines.append("⚠️ No indicator data available.")
